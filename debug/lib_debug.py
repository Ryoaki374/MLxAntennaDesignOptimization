...
def expected_improvement(kernel, x_new, X_sample, y_sample, Ky_opt_inv, length_scale, xi=0.01):
    mu, sigma = get_posterior(kernel, x_new, X_sample, y_sample, Ky_opt_inv, length_scale)
...
def lower_confidence_bound(kernel, x_new, X_sample, y_sample, Ky_opt_inv, length_scale, kappa=2.0):
    mu, sigma = get_posterior(kernel, x_new, X_sample, y_sample, Ky_opt_inv, length_scale)
...
def optimize_acquisition(kernel, acq_func, X_sample, y_sample, Ky_opt_inv, length_scale, lower_bounds, upper_bounds, acq_params):
    """
    ...
    def negative_acq_wrapper(x):
        val = acq_func(kernel, x, X_sample, y_sample, Ky_opt_inv, length_scale, **acq_params)
        return -val

    for i in range(n_restarts):
        x0 = np.random.uniform(lower_bounds, upper_bounds, dims)
        res = minimize(
            fun=negative_acq_wrapper, 
            x0=x0,
            bounds=bounds, 
            method='L-BFGS-B'
        )
...
def plot_gp_results(X_grid, true_func, X_init, y_init, best_x, best_y, mu, sigma, acq_values, strategy_name):
    ...
    ax1.scatter(X_init, y_init, c='w', s=100, alpha=1, edgecolors='k', lw=2, label='Initial Samples', zorder=3)
